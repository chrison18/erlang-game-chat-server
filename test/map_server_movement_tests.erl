    ...
    {setup,
     ...
     fun(MapPid) ->
         fun() ->
             ...
             ?assertMatch(#{shard := {5, 3}},
                          map_server:debug_role(MapPid, 1)),
             ?assertEqual(ok, map_server:move(MapPid, 1, down)),
             ...
             ?assertEqual(ok,
                          map_server:teleport(MapPid, 1, {80, 90})),
             ?assertEqual({ok, {80, 90}}, receive_result(MapPid, teleport)),
             Shards = maps:get(shards, map_server:debug_state(MapPid)),
             ?assertEqual(false, maps:is_key({5, 3}, Shards)),
             ?assertEqual([1], maps:get({40, 30}, Shards)),
             ?assertEqual(ok,
                          map_server:send_nearby(
                              MapPid, 1, <<"alice">>, <<"nearby">>)),
             ?assertEqual({ok, 1}, receive_result(MapPid, send_nearby)),
             assert_nearby_push(1, <<"alice">>, {80, 90}, <<"nearby">>),
             ?assertEqual(ok,
                          map_server:teleport(MapPid, 1, {100, 90})),
