...
项目使用 `erlc` 和 Shell 脚本，不使用 `rebar3`。服务端包含账号与在线角色管理、10 个固定聊天频道、3 个内部地图频道、8 个 `main` 广播 Worker、3 个地图 Worker、3 个附近聊天 Worker，以及每个连接独立的 `role_server`。固定地图为 `1`、`2`、`3`，大小均为 `100 x 100`。
...
登录默认进入地图 `1` 的 `{0,0}`。一个角色同时只能处于一张地图；地图聊天只广播给
...
普通公共频道和内部地图频道也各自在自己的 `channel_server` 内按 120ms 或 256 条组批。
普通频道沿用 `2010` 包装 `2009`，地图频道使用 `4015` 包装 `4014`。成员变化通过批内
偏移保持加入前、离开前消息边界，不触发全员提前发送。周围聊天直接读取地图 ETS，
再由每地图 Worker 按来源格子组批，使用 `4016` 包装 `4007`；私聊仍按单条推送。
自动循环会持续产生消息。应从很小的客户端数量开始验证，再逐步增加规模。

地图请求先由 `map_router` 根据 `MapId` 定位 `map_worker_1/2/3`，各地图的加入、退出、移动和传送互不排队；`map_router` 自身不串行业务请求。服务端 Shell 使用 `chat_metrics:snapshot().` 手动采集当前在线数、Role、频道、地图 Worker、世界/附近 Worker 邮箱，以及地图操作和批量统计；`map_worker_queues` 会分别给出地图 `1/2/3` 的邮箱长度。使用 `chat_metrics:print_online_clients().` 打印在线角色、邮箱长度、地图和坐标。接口只在调用时读取状态，不启动常驻统计进程。

当前架构、接口和流程图见 [Chat V1.3 设计文档](<docs/Chat V1.3 设计文档.md>)；旧负载的性能过程和 10000 客户端结果见 [Chat V1.2 调优报告](<docs/Chat V1.2 调优报告.md>)。V1.1 和 V1.2 设计文档作为历史版本保留。
